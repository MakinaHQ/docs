...
- Every other asset the strategy holds is valued _relative to_ the accounting token, through the [Oracle Registry](../oracles).
...
A Machine is linked to a set of **periphery** contracts, each of which can be a different implementation depending on the strategy's needs (see [Architecture → core vs. periphery](../architecture#core-vs-periphery)):
