...
Because the machine token is a plain ERC-20 whose value tracks a professionally managed, diversified strategy, it is useful well beyond simply holding:
...
The machine token is a standard ERC-20, so moving it to another chain relies on external bridging infrastructure such as [Wormhole NTT](https://ntt.wormhole.com/) rather than anything built into the token itself.

:::note
This is a separate, optional transport layer for the machine token. It is independent of the strategy's internal [cross-chain liquidity](../cross-chain/liquidity-bridging) and [accounting](../cross-chain/cross-chain-accounting) machinery, which move the _underlying_ capital between the Machine and its Spoke Calibers.
:::
