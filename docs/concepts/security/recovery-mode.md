...
**On each [Caliber](../caliber/overview):**

- Positions can only be **decreased or closed**. Opening or increasing a position is blocked.
- [Swaps](../caliber/swaps) may only convert **toward the accounting token**.

**On the [Machine](../machine/overview):**

- New **deposits are disabled**.
- Outbound **Hub → Spoke** [bridge transfers](../cross-chain/liquidity-bridging) are blocked.
...
Recovery Mode is **per-chain**. The Machine and Hub Caliber are governed on the Hub Chain, and **each Spoke Caliber must have Recovery Mode activated independently** on its own [Caliber Mailbox](../cross-chain/caliber-mailbox). A live incident may therefore need to be addressed on several chains.
