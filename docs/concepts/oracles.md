...
Everything Makina does rests on one ability: **valuing any token in terms of the [accounting token](architecture#glossary)**. Share price, AUM, loss checks, and risk caps are all denominated that way. The contract that provides this is the **Oracle Registry**, shared [infrastructure](architecture#shared-infrastructure) deployed once per chain and used by every strategy on it.
