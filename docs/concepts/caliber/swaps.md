...
A [Caliber](overview) frequently needs to convert one token into another: to assemble the [base tokens](base-tokens) a position requires, to realize [harvested](harvests) rewards, or to consolidate holdings. It does this through the **Swap Module**, a shared piece of [infrastructure](../oracles) that routes swaps through approved external routers and aggregators.
