...
The **Operator** is the entity that runs a strategy day to day. It is the actor with discretion (the one making the active-management decisions that generate returns), but that discretion is tightly bounded by the strategy's risk policy and the [pre-approved instruction set](../architecture/caliber/makina-vm).
...
- **Execute and maintain the strategy**: open, resize, and close [positions](../architecture/caliber/positions), [swap](../architecture/caliber/swaps) tokens, [harvest](../architecture/caliber/harvests) rewards, and [rebalance](../architecture/lifecycle#4-rebalance--harvest-the-strategy-is-actively-managed) across protocols and chains to deliver strong risk-adjusted returns.
- **Move liquidity**: allocate idle capital from the [Machine](../architecture/machine/overview) to Calibers and [bridge](../architecture/cross-chain/liquidity-bridging) it between the Hub and Spokes.
- **Settle redemptions**: free up accounting-token liquidity and [finalize the redemption queue](../architecture/machine/redemptions).
- **Keep accounting fresh**: re-account positions so the [share price](../architecture/machine/share-price) stays accurate.
...
The Operator works toward the strategy's published [mandate](../introduction#the-strategy-mandate), and is incentivized by [performance fees](../architecture/machine/fees) to maximize the long-term share price.
...
- **Execution** is limited to [pre-approved instructions](../architecture/caliber/makina-vm), and cannot call arbitrary contracts.
- **Swaps** may only output approved [base tokens](../architecture/caliber/base-tokens).
...
- It **cannot** withdraw user funds, send assets to arbitrary destinations, change its own limits, or mint shares to itself outside the [fee](../architecture/machine/fees) mechanism.
