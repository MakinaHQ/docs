...
The set of actions an [Operator](operator) may take is defined by the [Merkle root](../architecture/caliber/makina-vm#merkle-tree-permissioning) of allowed instructions stored on each [Caliber](../architecture/caliber/overview). Changing that root is the single most security-critical governance action in a strategy: it expands or contracts what the Operator can do. It therefore follows a deliberate, multi-stage, vetoable process.
...
4. **Timelock window.** The scheduled update is visible on-chain, and notifications go out to the Security Council and community channels. Users who disagree with the upcoming change have time to [redeem](../architecture/machine/redemptions) before it takes effect.
...
:::info[Implementation]
The on-chain entry points for scheduling, vetoing, and activating a root update live in [`Caliber.sol`](/contracts/core/caliber/contract.Caliber).
:::
