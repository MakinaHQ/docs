...
| `INFRA_CONFIG_ROLE`                | 1   | Configure shared core contracts (e.g. [oracle](../architecture/pricing-oracles) feed routes, cross-chain token mappings).                                                                                                                | DAO: 1-day delay                                        |
| `STRATEGY_DEPLOYMENT_ROLE`         | 2   | Deploy new strategies.                                                                                                                                                                                                                   | DAO: no delay                                           |
| `STRATEGY_COMPONENTS_LINKING_ROLE` | 3   | Link a strategy's contracts together (depositor, redeemer, fee manager, spoke calibers, bridge adapters), and disable or re-enable a spoke caliber.                                                                                      | DAO: 1-day delay on Hub Chain, no delay on Spoke chains |
| `STRATEGY_MANAGEMENT_CONFIG_ROLE`  | 4   | Designate the per-strategy entities (Operator, Risk Manager, Security Council, …), manage accounting agents, and authorize the CRE workflow IDs allowed to relay [spoke accounting](../architecture/cross-chain/cross-chain-accounting). | DAO: 2-day delay                                        |
...
| **[Risk Manager](risk-manager)**         | Schedules [instruction-root updates](root-update-lifecycle), sets the share supply cap, and manages whitelists and sanctions screening.                                                                                                                                                                                                          |
...
| **[Security Council](security-council)** | Vetoes scheduled changes, triggers [Recovery Mode](../security/recovery-mode), initiates [Security Module](../security/security-module) slashing, can force AUM updates, and can publish [spoke accounting snapshots](../architecture/cross-chain/cross-chain-accounting) directly as a CRE fallback. Becomes the Operator during Recovery Mode. |
