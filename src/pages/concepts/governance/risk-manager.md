...
- **Manage access screening**: the whitelists on the [Depositor](../architecture/machine/deposits#whitelisting), [Redeemer](../architecture/machine/redemptions#whitelisting), and [Pre-Deposit Vault](../architecture/machine/pre-deposit), and the [sanctions screening](../architecture/machine/deposits#sanctions-screening) toggle on the Depositor and Redeemer.

## Two speeds: immediate vs. timelocked

Not every change carries the same risk, so they don't all move at the same speed:

- A small number of **lower-risk settings** can be applied **immediately** by the Risk Manager, for example the share supply cap, the deposit/redemption whitelists, and sanctions screening.
