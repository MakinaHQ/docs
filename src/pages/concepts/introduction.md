...
What makes Makina different from a traditional smart-contract vault is the **execution model**. Instead of hard-coding each integration into the vault, Makina gives the Operator a generalized, permissioned execution engine ([the MakinaVM](architecture/caliber/makina-vm)) that can interact with almost any external protocol, but only through actions that governance has reviewed and explicitly approved in advance. This combines the flexibility of an actively managed fund with the safety of a tightly scoped vault.
...
As the strategy's total value grows, so does the value of each share. Holders capture returns through the appreciation of their [machine token](architecture/machine/machine-token), which is fully composable and can itself be used as collateral, paired in liquidity pools, or held in treasury. Operators and the protocol are compensated through [fees](architecture/machine/fees), aligning their incentive with long-term share-price growth.
...
- **Verifiable accounting.** [Share price](architecture/machine/share-price) is derived from the strategy's full, oracle-priced assets under management (including capital deployed on other chains), and is auditable by anyone at any time.
...
New to Makina? Read [Architecture](architecture/overview) and [Asset Lifecycle](architecture/lifecycle) in order. Together they give you a complete mental model in a few minutes.
:::
